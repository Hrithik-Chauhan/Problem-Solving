...
int main(){
    int num_of_cir,x,y,rad;
    cout<<"Enter x-coordinate of destination: ";
    cin>>x;
    cout<<"\n";
    cout<<"Enter y-coordinate of destination: ";
    cin>>y;
    ...
    cin>>rad;
    cout<<"\n";
    vector<int> cirX(num_of_cir);
    vector<int> cirY(num_of_cir);
    cout<<"Enter x-coordinates of circles center:\n";
    for(int i=0;i<num_of_cir;i++){
        cin>>cirX[i];
    }cout<<"\nEnter y-coordinates of circles center:\n";
    for(int i=0;i<num_of_cir;i++){
        cin>>cirY[i];
    }
    string ans=solve(x,y,num_of_cir,rad,cirX,cirY);
    cout<<ans;
    return 0;
}