...
public:
    ...
    int countpairsforrange(TrieNode* root,int n,int k){
    ...
    int countPairs(vector<int>& nums, int low, int high) {
        ...
        for(int i:nums){
            cnt+=countpairsforrange(newNode,i,high+1)-countpairsforrange(newNode,i,low);
            insert(newNode,i);
        }
    return cnt;
    }
...
int main(){
    ...
    cout<<"Enter range:\n";
    cin>>low>>high;
    int ans=a.countPairs(nums,low,high);
    cout<<ans;
    return 0;
}
